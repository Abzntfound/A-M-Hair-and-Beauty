...
(function(){
 ...
 if(path==='/cart/'||path==='/cart.html'){const css=document.createElement('link');css.rel='stylesheet';css.href='/cart-receipt.css';document.head.appendChild(css);}
 ...
 function pageEvents(){const params=new URLSearchParams(location.search),productId=params.get('id');if(path==='/'||path==='/index.html')track('homepage_view');if((path==='/products/'||path==='/products.html')&&productId)track('product_view',{productId});}
 if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',pageEvents,{once:true});else pageEvents();
 document.addEventListener('click',event=>{const add=event.target.closest?.('#add-to-cart-btn');if(add){const productId=new URLSearchParams(location.search).get('id'),quantity=Number(document.getElementById('qty-display')?.textContent)||1;if(productId)track('add_to_cart',{productId,quantity});}const checkout=event.target.closest?.('#normal-checkout');if(checkout)track('checkout_started');});
})();
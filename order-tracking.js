(() => {
  ...
  class AMOrderTracking extends HTMLElement {
    connectedCallback() {
      ...
      root.innerHTML = `
        <style>
          ...
          *{box-sizing:border-box}
          section{background:var(--bg);color:var(--text);padding:90px 5%;transition:.2s ease}
          .wrap{max-width:900px;margin:auto;text-align:center}
          .eyebrow{font-size:.78rem;font-weight:800;letter-spacing:.16em;text-transform:uppercase;color:#d946a6;margin-bottom:10px}
          h2{font-size:clamp(2rem,5vw,3.2rem);letter-spacing:-.045em;margin:0 0 12px}
          .intro{color:var(--muted);max-width:650px;margin:0 auto 30px;line-height:1.7}
          .panel{max-width:700px;margin:auto;background:var(--panel);border:1px solid var(--border);border-radius:26px;padding:26px;box-shadow:0 18px 55px var(--shadow);text-align:left}
          .grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}
          label{display:block;font-size:.84rem;font-weight:700;margin-bottom:7px;color:var(--text)}
          input{width:100%;border:1px solid var(--border);background:var(--field);color:var(--text);border-radius:13px;padding:13px 14px;font:500 14px Poppins,Arial,sans-serif;outline:none}
          input:focus{border-color:#d946a6;box-shadow:0 0 0 4px rgba(217,70,166,.1)}
          button{width:100%;margin-top:16px;border:0;border-radius:999px;padding:14px 18px;background:linear-gradient(135deg,#d946a6,#ec4899);color:#fff;font:800 14px Poppins,Arial,sans-serif;cursor:pointer}
          button:disabled{opacity:.6;cursor:wait}
          .result{display:none;margin-top:20px;padding:18px;border:1px solid var(--border);border-radius:16px;background:var(--field);color:var(--text)}
          .result.show{display:block}
          .result strong{display:block;margin-bottom:5px}
          .result p{margin:5px 0;color:var(--muted)}
          .track{display:inline-flex;margin-top:12px;padding:11px 16px;border-radius:999px;background:#151515;color:#fff;text-decoration:none;font-weight:700;font-size:13px}
          .error{color:#dc2626;font-weight:600}
          @media(max-width:650px){section{padding:65px 18px}.panel{padding:20px}.grid{grid-template-columns:1fr}}
        </style>
      ...
      const syncTheme = () => this.setAttribute('data-theme', document.documentElement.getAttribute('data-theme') === 'dark' ? 'dark' : 'light');
      syncTheme();
